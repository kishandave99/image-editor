        ...
        if (fileView.files && fileView.files.length) {
            for (var i = 0; i < fileView.files.length; i++) {
                ...
                if (!file.$error) {
                    ...
                    Upload.upload({
                        url: $rootScope.webService + 'uploadPhoto',
                        fields: {},
                        file: file
                    }).then(function (response) {
                        $scope.backGroundImage = $rootScope.publicResourceUrl + appUrl.resourceImageDownload + response.data.filename;
            ...
            for (var i = 0; i < fileView.files.length; i++) {
                ...
                if (!file.$error) {
                    ...
                    Upload.upload({
                        ...
                        $scope.logo.logoFileView = response.data;
